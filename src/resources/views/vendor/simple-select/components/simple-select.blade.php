        ...
        @isset($attributes['multiple'])            
            <div x-show="selected && typeof selected === 'object' && selected.length > 0" class="flex flex-wrap space-x-1">
                <template x-for="(value, index) in selected" :key="index">
                    <div class="text-white rounded-full truncate bg-[#DC2626] px-2 py-0.5 my-0.5 flex flex-row items-center">
            ...
            >
                ...
                <template x-for="(option, index) in Object.values(options)" :key="index">
                    <li
                        ...
                        @isset($attributes['multiple'])
                            x-bind:class="{
                                'bg-[#DC2626] text-white hover:none': selected && selected.includes(getOptionValue(option, index)),
                                'text-gray-900 cursor-default hover:text-white hover:bg-[#DC2626] hover:cursor-pointer focus:bg-gray-200': !(selected && selected.includes(getOptionValue(option, index))),
                            }"
                        @else
                            x-bind:class="{
                                'bg-gray-300 text-black hover:none': selected == getOptionValue(option, index),
                                'text-gray-900 cursor-default hover:text-white hover:bg-[#DC2626] hover:cursor-pointer focus:bg-gray-200': !(selected == getOptionValue(option, index)),
                            }"
                        @endisset
